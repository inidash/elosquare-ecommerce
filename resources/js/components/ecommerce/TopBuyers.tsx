...
export default function TopBuyers() {
    const { top_users} = usePage().props as any;
    return (
        // <div className="bg-white py-12">
        <section className="py-16 bg-gradient-to-br from-blue-50 via-indigo-200 to-white relative overflow-hidden">
      {/* Background Elements */}
        ...
        <div className="flex flex-col md:flex-row  items-center md:justify-between mb-12">
          <div className="flex items-center gap-4">
            <div className="relative">
              <div className="absolute inset-0 bg-gradient-to-r from-blue-300 to-gray-300 rounded-2xl blur opacity-20" />
              <div className="relative bg-gradient-to-r from-blue-300 to-gray-300 p-3 rounded-2xl">
                <Flame className="w-8 h-8 text-white" />
              </div>
            </div>
            <div>
              <h2 className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-blue-600 via-gray-600 to-purple-600 bg-clip-text text-transparent mb-2">
                Leading Buyers
              </h2>
              <p className="text-lg text-slate-600 font-medium">
                A short list of leading buyers in the marketplace.
              </p>
            </div>
          </div>
            <Link href='/top-buyers'>
                    ...
                    <div className="swiper-wrapper items-center py-4">
                        ...
                        >
                            {top_users.map((user, index) => (
                                <SwiperSlide key={user.id} className="swiper-slide p-4 text-center">
                                    <div className="relative flex flex-col h-60 items-center rounded-lg bg-gray-500 p-6">
                                        ...
                                        <p className='py-2'><strong>Buyer State:</strong> {user.address}</p>
                                        <p className=' py-2'><strong>Buyer Points:</strong> {user.points}</p>
                                        <p className=' py-2'><strong>Products Bought:</strong> {user.transactions_count}</p>
                                        {index === 0 && (
