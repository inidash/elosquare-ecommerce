  ...
  return (
    ...
    >
      ...
      <div style={{ textAlign: 'center', marginBottom: 8 }}>
        ...
        <div className="text-xs">******************************</div>
        <div style={{ fontWeight: 'bold', fontSize: 16, margin: '6px 0' }}>{('Invoice')}</div>
      ...
      <div className="mb-2 text-xs">
        ...
        +234 (704) 3007 794
        <br />
        {('Address')}: No. 298, isheri idimu road Alimosho, Lagos State, Nigeria.
      </div>
        ...
        {order.order_items.map((item) => (
          <div key={item.id} className="mb-1">
            ...
            <div className="flex items-center justify-between text-xs">
              <span>
                {item.quantity} {('pcs')} x {formatPrice(item.unit_price)}
              </span>
      ...
      {order.discount_amount > 0 && (
        <div className="mb-1 flex items-center justify-between text-xs text-red-600">
          <span>{('Discount')}:</span>-{formatPrice(order.discount_amount)}
        </div>
      )}
      ...
      <div style={{ textAlign: 'center', margin: '10px 0' }}>
        <div className="mt-1 text-xs text-gray-600">{('All amounts in USD')}</div>
        <div className="mt-1 text-xs font-bold">{('THANK YOU FOR YOUR ORDER!')}</div>
